...
public class Perpendicular : MonoBehaviour
{
    ...
    public void solveQuestion()
    {
        if (!InExplain)
        {
            ...
            {
                Answer.text = "";
                Answer.text += $"<line-height=15%>{FractionCalculator.GetStringFrec("m2 = ", "   -a", "     b")}</line-height>\n";
                Answer.text += $"<line-height=15%>{FractionCalculator.GetStringFrec("m2 = ", "  -" + X.text, "  " + Y.text)}</line-height>\n";
                Answer.text += $"<line-height=15%>{FractionCalculator.GetStringFrec("since L1 _|_ L2 , m = ", "     -1", "     m2")}</line-height>\n";
        ...
        while (i < written.Length)
        {
            if (written[i].Equals('#'))
            {
                ...
                else
                {
                    ...
                    while (!IsAnimatedPart)
                        yield return null;
                    Answer.text += FractionCalculator.GetStringFrec("since L1 _|_ L2, m = ", "     -1" , "     m<sub style='font-size: larger;'>2</sub>");
                    i = written.Length;
                }
            }
            i++;
    ...
    public void CalcSlope()
    {
        ...
        voiceClips = Resources.LoadAll<AudioClip>("AsemEng/Q5");
        int nueInt = -int.Parse(X.text);
        int DenoInt = -int.Parse(Y.text);
        FractionCalculator.SimplifyFraction(ref DenoInt, ref nueInt);
        int TempNue = nueInt;
        nueInt = DenoInt;
        DenoInt = TempNue;
        nue = nueInt.ToString();
